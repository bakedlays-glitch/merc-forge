...
def load_8bit_sti(file):
    ...
    colors_in_right_order = [color for rgb in palette_colors for color in rgb]
    palette = ImagePalette.ImagePalette("RGB", colors_in_right_order)
...
class StiImagePlugin(ImageFile.ImageFile):
    ...
    def _save_etrle_images(img, fd):
        ...
        # write image data
        fd.seek(StiHeader.get_size(), 0) # from start
        palette_bands = palette.tobytes().ljust(256 * 3, b'\x00')
        palette_colors = palette_bands
...
class StiImageEncoder(PyEncoder):
    ...
    def init(self, args):
        ...
        if self.do == 'colors':
            ...
            if alpha_mask == 0:
                self._mode = 'RGB' # force no alpha
            else:
                self._mode = 'RGBA' # force alpha
